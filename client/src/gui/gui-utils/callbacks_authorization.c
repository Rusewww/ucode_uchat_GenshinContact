...
void mx_reset_auth(GtkNotebook *note, GtkWidget *page, guint page_num, GtkBuilder *builder) {
    ...
    mx_entry_set_icon_by_path(GTK_ENTRY(signup), MX_HIDDEN, GTK_ENTRY_ICON_SECONDARY);
...
void mx_show_password(GtkEntry *entry, GtkEntryIconPosition icon_pos, GdkEvent *event, gpointer *entry_second) {
    if (gtk_entry_get_visibility(entry)) {
        mx_entry_set_icon_by_path(entry, MX_HIDDEN, icon_pos);
