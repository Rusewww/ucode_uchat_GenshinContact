...
void mx_delete_selected_msgs(GtkButton *btn, t_chat *chat) {
    ...
    gtk_list_box_selected_foreach(groom->box_messages, (GtkListBoxForeachFunc) req_delete_msg, chat);
    (void) btn;
}

void mx_show_edit_msg(GtkButton *btn, t_chat *chat) {
    GObject *buf = gtk_builder_get_object(chat->builder, "buffer_message");
    ...
    GObject *btn_stick = gtk_builder_get_object(chat->builder, "btn_choose_sticker");
    GObject *box_stick = gtk_builder_get_object(chat->builder, "box_stickers");
    ...
    gtk_text_buffer_set_text(GTK_TEXT_BUFFER(buf), msg->msg, -1);
