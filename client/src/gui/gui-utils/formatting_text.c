...
gchar *mx_format_text(gchar *text) {
    gchar *res = NULL;
    gchar *save = NULL;
    res = set_tag(text, MX_OP_UNDER, MX_FT_UNDER);
    save = res;
    res = set_tag(res, MX_OP_BOLD, MX_FT_BOLD);
    g_free(save);
    save = res;
    res = set_tag(res, MX_OP_SCRATCH, MX_FT_SCRATCH);
    g_free(save);
    save = res;
    res = set_tag(res, MX_OP_ITALIC, MX_FT_ITALIC);
    g_free(save);
    save = res;
    res = set_tag(res, MX_OP_IMPORTANT, MX_FT_IMPORTANT);
    g_free(save);
    return res;
}
