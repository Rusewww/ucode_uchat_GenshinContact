...
void mx_show_user_info(GtkBuilder *build, gchar *login, gchar *desc) {
    GObject *login_info = gtk_builder_get_object(build, "label_user_info_login");
    GObject *desc_info = gtk_builder_get_object(build, "buffer_user_desc");
    gtk_text_buffer_set_text(GTK_TEXT_BUFFER(desc_info), desc, -1);
    gtk_label_set_text(GTK_LABEL(login_info), login);
}

void mx_req_get_member_info(GObject *up, t_chat *chat) {
    t_dtp *dtp = mx_member_info_request((unsigned long) g_object_get_data(up, "member_id"));
