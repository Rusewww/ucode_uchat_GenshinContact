...
void mx_set_room_members(GtkBuilder *builder, t_groom *groom) {
    GObject *listbox_sett = gtk_builder_get_object(builder, "listbox_sett_members");
    GObject *listbox_info = gtk_builder_get_object(builder, "listbox_info_members");
    ...
    g_hash_table_foreach(groom->members, (GHFunc)mx_add_to_sett_members, builder);
    g_hash_table_foreach(groom->members, (GHFunc)mx_add_to_info_members, builder);
}
