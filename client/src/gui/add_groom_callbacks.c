...
void mx_set_current_room_sett(GtkBuilder *builder) {
    t_groom *groom = mx_get_selected_groom(builder, "listbox_rooms");
    ...
    GObject *header_name = gtk_builder_get_object(builder, "label_header_roomname");
    ...
    if (groom) {
        ...
        char *value = g_strdup_printf("%.4f Wt", groom->power);
...
void mx_select_room(GtkWidget *event_box, GdkEventButton *event, gpointer *user_data) {
    ...
    GObject *btn_room_sett = gtk_builder_get_object(data->chat->builder, "btn_show_room_sett");
...
void mx_show_join_to_room(GtkWidget *event_box, GdkEventButton *event, gpointer *user_data) {
    t_signal_data *data = g_object_get_data(G_OBJECT(event_box), "sigdata");
    t_groom *lgroom = mx_get_selected_groom(data->chat->builder, "listbox_rooms");
    GObject *label_name = gtk_builder_get_object(data->chat->builder, "label_join_roomname");

    gtk_list_box_select_row(data->groom->box_rooms, data->groom->row_room);
    mx_reset_messege_room(lgroom, data->chat->builder);
    mx_widget_switch_visibility_by_name(data->chat->builder, "dialog_join_to_room");
