...
static void show_ban_member(GObject *btn, GtkBuilder *builder) {
    ...
    g_object_set_data(btn_ban, "member_id", g_object_get_data(btn, "member_id"));
    mx_widget_set_visibility(GTK_WIDGET(dialog), TRUE);
    gtk_label_set_text(GTK_LABEL(label_member), g_object_get_data(btn, "member_login"));
}

void mx_add_to_sett_members(gint *key, char *value, GtkBuilder *builder) {
    t_groom *groom = mx_get_selected_groom(builder, "listbox_rooms");

    if (g_strcmp0(groom->customer, value)) {
        GObject *listbox = gtk_builder_get_object(builder, "listbox_sett_members");
...
static void show_user_info(GObject *btn, GtkBuilder *builder) {
    ...
    g_object_set_data(popup, "member_id", g_object_get_data(btn, "member_id"));
