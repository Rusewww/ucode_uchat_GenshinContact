...
static t_groom *help_create_message(gboolean *valid, cJSON *msg, t_gmsg *gmsg, t_chat *chat) {
    cJSON *data = NULL;
    t_groom *groom = NULL;
    ...
    if (*valid && (*valid = get_data(msg, &data, "user_id", cJSON_IsNumber))) {
        void *hash = g_hash_table_lookup(groom->members, GINT_TO_POINTER(data->valueint));
...
t_gmsg *mx_create_gmsg(cJSON *msg, t_chat *chat) {
    ...
    cJSON *data = NULL;
    gboolean valid = TRUE;
    help_create_message(&valid, msg, gmsg, chat);
    if (valid) {
        if ((valid = get_data(msg, &data, "message_id", cJSON_IsNumber)) == 1) {
            gmsg->message_id = data->valuedouble;
        }
        if ((valid = get_data(msg, &data, "msg_type", cJSON_IsNumber)) == 1) {
            gmsg->type = data->valuedouble;
        }
        if ((valid = get_data(msg, &data, "power", cJSON_IsNumber)) == 1) {
            gmsg->power = 0.026751 * data->valuedouble;
        }
    } else {
        if (mx_delete_gmsg(gmsg)) {
            return NULL;
        }
    }
    gmsg->row_msg = NULL;
