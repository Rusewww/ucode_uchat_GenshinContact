...
gboolean mx_authorization_handler(t_dtp *token, t_chat *chat) {
    ...
    if (chat->auth_token || !cJSON_IsString(auth_token) || !cJSON_IsString(login) ||
        !mx_isvalid_token(auth_token->valuestring) || !mx_match_search(login->valuestring, MX_LOGIN_REGEX)) {
        return FALSE;
    }
