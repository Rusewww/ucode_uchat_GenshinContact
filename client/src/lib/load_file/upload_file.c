...
static gboolean is_ready(GSocketConnection *conn) {
...
static void send(t_dtp *request, GFile *file, t_chat *chat) {
    ...
    if (is_ready(conn)) {
        g_output_stream_splice(out, G_INPUT_STREAM(file_in), G_OUTPUT_STREAM_SPLICE_CLOSE_SOURCE, NULL, NULL);
    }
...
void mx_upload_file(gchar *path, gint room_id, t_chat *chat) {
    ...
    t_dtp *file_request = mx_upload_file_request(g_file_info_get_name(info), g_file_info_get_size(info),
                                                 chat->auth_token, room_id);
    if (g_file_info_get_size(info) <= MX_MAX_FILE_SIZE)send(file_request, file, chat);
    g_object_unref(info);
}
