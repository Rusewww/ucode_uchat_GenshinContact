...
t_db_user *mx_get_user_by_login(sqlite3 *db, char *login) {
    ...
    rv = sqlite3_prepare_v3(db, "select * from users where login = ?1", -1, 0, &stmt, NULL);
...
t_db_user *mx_get_user_by_id(sqlite3 *db, int user_id) {
    ...
    rv = sqlite3_prepare_v3(db, "select * from users where id = ?1", -1, 0, &stmt, NULL);
...
t_db_user *mx_get_user_by_token(sqlite3 *db, char *token) {
    ...
    rv = sqlite3_prepare_v3(db, "select * from users where token = ?1", -1, 0, &stmt, NULL);
...
t_db_room *mx_get_room_by_id(sqlite3 *db, int id) {
    ...
    mx_error_sqlite(sqlite3_prepare_v3(db, "select * from rooms where " "id = ?1", -1, 0, &stmt, NULL), "get room by id");
