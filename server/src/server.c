...
static void message_ready(GObject *source, GAsyncResult *res, gpointer data) {
    GDataInputStream *in = G_DATA_INPUT_STREAM(source);
    gsize count = 0;
    t_client *cli = (t_client *) data;
    if (!g_socket_connection_is_connected(cli->conn) || g_output_stream_is_closed(G_OUTPUT_STREAM(cli->out)) ||
        g_input_stream_is_closed(G_INPUT_STREAM(in))) {
        mx_de_init_client(&cli);
        return;
    }
...
static gboolean incoming(GSocketService *service, GSocketConnection *conn, GObject *source, gpointer data) {
